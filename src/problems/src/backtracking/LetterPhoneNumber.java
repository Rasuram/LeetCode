package problems.src.backtracking;
...
public class LetterPhoneNumber {
  ...
  public static void main(String[] args) throws Exception {
    List<String> result = new LetterPhoneNumber().letterCombinations("45");
    result.forEach(System.out::println);
  }

  private List<String> letterCombinations(String digits) {
    String first = NUMBER_ALPHA[Integer.parseInt(digits.split("")[0])];
    String second = NUMBER_ALPHA[Integer.parseInt(digits.split("")[1])];
    List<String> prev = new ArrayList<>();
    for (int i = 0; i < first.length(); i++) {
      for (int j = 0; j < second.length(); j++) {
        String s = String.valueOf(first.charAt(i)) + second.charAt(j);
        prev.add(s);
      }
    }
    return prev;
